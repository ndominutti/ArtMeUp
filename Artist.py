...
class Artist():
    ...
    def __init__(self, img_size=(400,400), random_seed=272, pretrained_model_path=None):
        
        self.img_size = img_size
        if pretrained_model_path is None:
            pretrained_model_path = 'model/vgg19_weights_tf_dim_ordering_tf_kernels_notop.h5'
    ...
    def load_images(self, content_img_path=None, style_img_path=None):
        ...
        content_image = np.array(Image.open(content_img_path).resize((img_size[0], img_size[1])))
        content_image = tf.constant(np.reshape(content_image, ((1,) + content_image.shape)))
        
        style_image =  np.array(Image.open(style_img_path).resize((img_size[0], img_size[1])))
    ...
    def initialize(self, content_img_path=None, style_img_path=None, content_layer=[('block5_conv4',1)], style_layers=None):
        '''
    ...
    def train(self,epochs=1000, alpha=10, beta=40,verbose=False, 
                  verbose_step=250, save_images=True, save_step=250, save_path=None,
                  plot=False, plot_step=250, fig_size=(7,7)):
        '''
        '''
        self.preprocess()
        self.set_optimizer()
        ...
        for i in range(epochs):
            ...
            if plot:
                if i%plot_step == 0:
                    ...
                    if i==0:
                        image = Image.open(self.content_img_path).resize((self.img_size[1], self.img_size[0]))
                    else:
...
class ImgsToVideo():
    ...
    def read_images(self):
        ...
        try:
            ...
            raise Exception('Images must be ordered in numerical ascending way (1,2,3...,n)')
